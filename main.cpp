...
void visit(const int city, int hops, int lenth, int *path)
{
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                visit(i, hops+1, lenth+Cost[i][city], path);
            }
        }
...
Tour* addCity(Tour *tour, int newcity)
{
    ...
    *newTour = new Tour[1];
    ...
    for(int i = 0; i < n+1; i++)
    {
        (*newTour)->cities[i] = tour->cities[i];
    }
...
inline bool visited(Tour *tour, int city)
{
    ...
    {
        if(tour->cities[i] == city) return true;
    }
...
int Create_Queues()
{
    ...
    while(num < comm_sz+NThreads && !local_queue.empty())
    {
        ...
        for(int i = 1; i < n; i++)
        {
            if(adj[temp->cities[temp->num_cities - 1]][i] && !visited(temp, i))
            {
